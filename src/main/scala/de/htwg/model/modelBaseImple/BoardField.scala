package de.htwg.model.modelBaseImple
  ...
  trait BoardField {
    def accept(visitor: FieldVisitor): Int
    val name: String
    val index: Int
  }
  ...
  case class PropertyField(
    ...
    color: PropertyField.Color,
    mortgage: PropertyField.Mortgage = PropertyField.Mortgage(),
    house: PropertyField.House = PropertyField.House()
  ) extends BoardField with BuyableField {
    override def withNewOwner(player: IPlayer): PropertyField = this.copy(owner = Some(player))
    override def accept(visitor: FieldVisitor): Int = visitor.visit(this)
  }
