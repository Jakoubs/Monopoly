          ...
          onAction = _ => {
            ...
            selectedPlayerOption match {
              case Some(selectedPlayerName) =>
                ...
                def findSelectedPropertyNames(node: javafx.scene.Node): List[String] = {
                  node match {
                    case vbox: javafx.scene.layout.VBox =>
                      vbox.getChildren.asScala.toList.flatMap(child => findSelectedPropertyNames(child.asInstanceOf[javafx.scene.Node]))
                    case hbox: javafx.scene.layout.HBox =>
                      hbox.getChildren.asScala.toList.flatMap(child => findSelectedPropertyNames(child.asInstanceOf[javafx.scene.Node]))
                    case cb: javafx.scene.control.CheckBox if cb.isSelected =>
                      List(cb.getId)
                    case _ => Nil
                  }
                }

                val selectedOfferPropertyNames = findSelectedPropertyNames(offerBox.delegate)
                val listOffer = selectedOfferPropertyNames.flatMap { propId =>
                  controller.currentPlayer.getProperties(controller.board.fields)
                    .find(_.name == propId)
                }

                val selectedDemandPropertyNames = findSelectedPropertyNames(demandBox.delegate)
                val listDemand = selectedDemandPropertyNames.flatMap { propId =>
                  ...
                  tradeResult match {
                    case Some((p1, p2, updatedBoard)) =>
                      ...
                      val updatedPlayers = controller.players.map {
                        case player if player.name == p1.name => p1
