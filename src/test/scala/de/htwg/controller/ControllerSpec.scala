...
class ControllerSpec extends AnyWordSpec with Matchers {
  ...
  "A Controller" should {
    ...
    "detect game over if only one player has money" in {
      ...
      val gameOverController = new Controller(MonopolyGame(Vector(brokePlayer, richPlayer), board, richPlayer, sound = false))
    ...
    "return correct player status string" in {
      ...
      val testController = new Controller(game)
    ...
    "correctly return owned properties grouped by player" in {
      ...
      val testController = new Controller(testGame)
