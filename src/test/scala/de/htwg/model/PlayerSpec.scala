...
class PlayerSpec extends AnyWordSpec {
  ...
  "Player" should {
    ...
    "remain in jail when trying to move while jailed" in {
      val player = Player("TestPlayer", 2000, position = 10, isInJail = true,0)
    ...
    "move to jail after rolling doubles three times" in {
      val player = Player("TestPlayer", 1500, position = 10, isInJail = false, 0)
    ...
    "never be on index 0" in {
      val player = Player("TestPlayer", 2000, position = 35, isInJail = false, 0)
    ...
    "change balance when collecting money" in {
      val player = Player("TestPlayer", 1000, position = 35, isInJail = false, 0)
